...
import { DiamondService } from '@/services/adminServices/diamond.service';
import { DiamondService as CommonDiamondService } from '@/services/diamond.service';
import { NextFunction, Request, Response } from 'express';
import Container from 'typedi';

export class DiamondController {
  private commonDiamondService = Container.get(CommonDiamondService);
  private diamondService = Container.get(DiamondService);

  public getDiamondList = async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  public getDiamond = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.success(ResponseMessages.DIAMOND_FOUND, diamond);
    } catch (error) {
      next(error);
    }
  };
  ...
  public uploadViaFile = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const file = (req as MulterRequest).file;
      await this.diamondService.uploadViaFile(file);

      res.success(ResponseMessages.UPLOAD_VIA_FILE_SUCCESS);
    } catch (error) {
      next(error);
    }
  };

  public getDiamondUploadFile = async (req: Request, res: Response, next: NextFunction) => {
    try {
